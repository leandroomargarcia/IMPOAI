...
**Order:** measure the graph first (this doc). Build the HTTP API later. OpenTelemetry is for that API, not for these 50 in-process calls.

## Gold set

- **50 cases**: `question` (as the graph expects it, including CIF) + **verified 8-digit NCM** from the catalog (not eyeballed).
- About **35 typical** products across chapters we care about; about **15 edge** (numeric threshold, residual “las demás”, BK/BIT, mixed/used).
...
- **LangSmith** (`LANGSMITH_TRACING=true`, project `IMPOAI` in `.env`): one trace per invoke, one span per node, prompt/completion, span duration, tokens. On for gold; off for mocked pytest.
...
Do not stand up Langfuse next to LangSmith.
...
1. **Now:** per-node and total ms from LangSmith spans (and the gold JSON). That is graph time; that is what the 50 calls can teach.
2. **After the office is the product to expose:** wrap the same `invoke` in an HTTP API.
3. **Then:** OpenTelemetry at the **gateway** (rate, errors, duration of the request). Propagate `trace_id` into LangSmith so one user call = one HTTP span + the same graph trace.

Instrumenting OTel before the API duplicates LangSmith on the graph and still leaves gateway latency at zero.
